...
## ORM-модели
ORM-модели находятся в `webapp/src/models`.  Для их использования в Celery требуется копирование в каталог `celery/src`, что осуществляется через bash-скрипт `celery/copy_models.sh`. 

Этот подход выбран из-за его простоты и удобства: создание отдельного пакета не требуется, поскольку исходные коды для Celery и webapp находятся в одном репозитории.
